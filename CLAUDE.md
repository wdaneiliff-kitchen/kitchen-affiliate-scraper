...
- **Main scrape:** `.github/workflows/scrape-and-upload.yml` runs on cron 13 times/day, with each cron firing 17 min *before* its dashboard target so the ~13 min scrape finishes at the target tick (e.g. cron at 6:43am → row written by ~6:56am for the 7am clock-tick). Dashboard targets in CDT: 7am, 8:30am, 10am, 11:30am, 1pm, 2:30pm, 4pm, 5:30pm, 7pm, 8:30pm, 10pm, 11pm, 11:45pm. GH Actions cron has no timezone support so the schedule slides 1 hour earlier in CST. **Minute marks are :13/:43/:28 (not :15/:45/:30) on purpose** — common quarter-hour marks are heavily contended in GH Actions' free-tier queue and we saw 2-4 hour cron delays on those marks in May 2026. Off-quarter marks consistently fire within 30 min. If delays ever come back: external trigger via cron-job.org or a small VPS POSTing to `workflow_dispatch` bypasses GH cron entirely.
...
- **Google Sheets tabs have a fixed grid `rowCount` and silently reject writes past it.** Each tab has a grid size (default 1000 rows). Once data fills the grid, every subsequent `values.append` or `values.update` past `A{rowCount+1}` fails with `Range (Comissions!AN) exceeds grid limits. Max rows: N-1`. On 2026-05-20 the Comissions tab hit 2261/2261 — 5 scheduled scrapes failed before manual expansion. Fix landed same day (`ensureGridRoom` helper in `packages/shared/src/sheets.js`, called from every write path in `shared/sheets.js`, `rpm/src/sheets.js`, `shared/drift-audit.js`, `shared/audit-aggregates.js`). The helper checks free rows before each write and calls `appendDimension` to grow by `needed + 1000` if short. If you ever add a new write path to a sheet tab, call `ensureGridRoom(sheets, spreadsheetId, tabName, plannedRows)` before the write. Expansion is non-destructive (rows added at the bottom, existing data untouched, invisible to Looker since dashboards filter by `order_date`).
