#!/usr/bin/env node
/**
 * One-time migration: convert date columns in the Comissions sheet from UTC to Central time.
 * Date columns: order_date (3), click_date (8), validation_date (9), modified_date (10)
 */
...
async function main() {
  ...
  const updatedRows = rows.map((row, i) => {
    ...
    for (const col of DATE_COLS) {
      ...
      const converted = utcStringToCentral(original);
  ...
  if (changed === 0) {
    console.log('✅ No dates needed conversion (already in Central time or empty).');
    return;
  }
